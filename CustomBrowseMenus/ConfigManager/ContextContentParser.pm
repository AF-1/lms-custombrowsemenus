# ConfigManager::ContextContentParser
#
# (c) 2021 AF
#
...
package Plugins::CustomBrowseMenus::ConfigManager::ContextContentParser;
...
sub checkContent {
	...
	if (defined($xml->{'menu'}) && defined($xml->{'menu'}->{'id'})) {
		my $enabled = $prefs->get('context_menu_'.escape($xml->{'menu'}->{'id'}).'_enabled');
