...
package Plugins::CustomBrowseMenus::MenuHandler::FunctionCmdPlay;
		...
		if (scalar(@functions) > 0) {
			...
			if ($dataFunction =~ /^(.+)::([^:].*)$/) {
				...
				if (UNIVERSAL::can("$class", "$function")) {
					...
					no strict 'refs';
					$result = eval { &{"${class}::${function}"}($client, \%parameters, $cmd) };
					if ($@) {
						$self->logHandler->warn("Error calling ${class}->${function}: $@");
					}
				} else {
					$self->errorCallback->("Error calling ${class}->${function}: function does not exist");
				}
			}
		}
	} else {
