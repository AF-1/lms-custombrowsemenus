...
package Plugins::CustomBrowseMenus::MenuHandler::BaseMenuHandler;
...
sub getMenuItems {
	my ($self, $client, $item, $context, $interfaceType) = @_;
...
sub _getFunctionMenu {
	my ($self, $client, $item, $context, $parameters, $interfaceType) = @_;
...
sub _getMenuItems {
	my ($self, $client, $item, $option, $mainBrowseMenu, $context, $interfaceType, $params) = @_;
	my @listRef = ();
		...
		for my $menu (keys %$browseMenus) {
			...
			if ($browseMenus->{$menu}->{'enabled'}) {
				if ($self->_isMenuEnabledForClient($client, $browseMenus->{$menu}) &&
						$self->_isMenuEnabledForVirtualLibrary($client, $browseMenus->{$menu}) &&
		...
		if (ref($item->{'menu'}) eq 'ARRAY') {
			foreach my $it (@{$item->{'menu'}}) {
				if ($self->_isMenuEnabledForClient($client, $it) &&
						$self->_isMenuEnabledForVirtualLibrary($client, $it) &&
			...
			if ($self->_isMenuEnabledForClient($client, $item->{'menu'}) &&
					$self->_isMenuEnabledForVirtualLibrary($client, $item->{'menu'}) &&
...
sub _combineKeywords {
	my ($parentKeywords, $optionKeywords, $selectionKeywords) = @_;
...
sub _isMenuEnabledForVirtualLibrary {
	my ($self, $client, $menu) = @_;

	my $persistentIDforActiveClientVL = undef;
	if (defined($client)) {
		$persistentIDforActiveClientVL = Slim::Music::VirtualLibraries->getLibraryIdForClient($client);
	}
	if (defined($menu->{'includedlibraries'})) {
		if (defined($persistentIDforActiveClientVL)) {
			my @libraries = split(/,/, $menu->{'includedlibraries'});
			for my $libraryPersistentID (@libraries) {
				if ($persistentIDforActiveClientVL eq Slim::Music::VirtualLibraries->getRealId($libraryPersistentID)) {
					return 1;
				}
			}
		}
...
sub getPageItem {
	my ($self, $client, $params, $contextParams, $checkContextType, $interfaceType) = @_;
...
sub getPageItemsForContext {
	...
	if (defined($items) && ref($items) eq 'ARRAY') {
		...
		$self->logHandler->debug("Getting item data for ".$count." items");
		$count = 0;
	...
	$self->logHandler->debug("Returning ".(scalar(@{$result{'items'}}))." items");

	return \%result;
}

sub displayAlbumAsHTML {
	my ($self, $album, $form, $descend, $sort) = @_;
...
sub displayTrackAsHTML {
	my ($self, $track, $form, $interfaceType) = @_;
...
sub getContext {
	my ($self, $client, $params, $realNames) = @_;
	my @result = ();
...
sub _getSubContext {
	my ($self, $client, $params, $groups, $currentItems, $level, $parameterContainer, $realNames) = @_;
	my @result = ();
...
sub standardTitle {
	my ($self, $client, $track, $format) = @_;

	if (!defined($format)) {
		$format = Slim::Music::Info::standardTitleFormat($client) || 'TITLE';
	}
	$format =~ s/\bARTIST\b/CUSTOMBROWSEMENUS_ARTIST/g;
	$format =~ s/\bCOMPOSER\b/CUSTOMBROWSEMENUS_COMPOSER/g;
	$format =~ s/\bBAND\b/CUSTOMBROWSEMENUS_BAND/g;
	$format =~ s/\bCONDUCTOR\b/CUSTOMBROWSEMENUS_CONDUCTOR/g;
...
sub showBrieflyPlayStatus {
	my ($self, $client, $displayString, $item) = @_;
...
sub _playTracks {
	my ($self, $client, $command, $trackIds) = @_;
